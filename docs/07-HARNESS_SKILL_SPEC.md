...
This repository treats each specialist `skills/harness-<phase>/SKILL.md` as the canonical phase source. Router fallback references are generated by `scripts/sync-harness-phases.mjs`; they MUST NOT be edited independently. Run `node scripts/sync-harness-phases.mjs --check` to detect drift.
