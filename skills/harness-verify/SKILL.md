...
name: harness-verify
description: >-
  Design, create, migrate, or repair agent-facing repository verification by
  exposing thin quick, affected, and full entry points that delegate to native
  lint, type, test, build, and workspace tooling; conservatively map changed
  files; compose deterministic garden checks; preserve failures; and report
  trustworthy evidence. Use when verification commands are fragmented, slow,
  unclear, inconsistent between local and CI use, lack a stable affected mode,
  or need migration from harness-slim. Do not use merely to run already-clear
  checks for an ordinary coding task, define feature acceptance or garden
  invariants, debug product failures, or redesign CI broadly.
---
