...
Thank you for helping improve harness-creator. Contributions should keep the skill dependency-free, preserve the five-subsystem harness contracts, and make behavior or documentation easier to verify.

## Before opening a change

1. Read `skills/harness-creator/SKILL.md` and the relevant reference files under `skills/harness-creator/references/`.
...
TMPDIR=$(mktemp -d)
node skills/harness-creator/scripts/create-harness.mjs --target "$TMPDIR"
node skills/harness-creator/scripts/validate-harness.mjs --target "$TMPDIR"
rm -rf "$TMPDIR"
```
...
- Update `README.md` or `skills/harness-creator/README.md` when user-facing paths or usage change.
